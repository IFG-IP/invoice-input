...
Vertex AI APIを使う場合は、Google Cloud CLIで認証したうえで以下の形式にします。
...
一時的にアクセストークンを直接使う場合は、`.env` に `VERTEX_ACCESS_TOKEN=...` を追加できます。サービスアカウントに紐づけたAPIキーを使う場合は `VERTEX_API_KEY=...` を使えます。
